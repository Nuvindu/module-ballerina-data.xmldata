...
public class SequenceInfo implements ModelGroupInfo {
    ...
    private boolean isFieldOptional(String element) {
        ...
        // xmlElementNameMap maps xmlName -> fieldName, so we need to get the field name
        String fieldName = xmlElementNameMap.getOrDefault(element, element);
        Field field = fieldType.getFields().get(fieldName);
        if (field == null) {
            return false;
        }
        // A field is considered optional for sequence validation if:
        // 1. The field has the OPTIONAL flag (declared with ? after field name), OR
        // 2. The field is annotated with @Any (since @Any fields may produce no XML elements)
        if (SymbolFlags.isFlagOn(field.getFlags(), SymbolFlags.OPTIONAL)) {
            return true;
        }
        // Check if this is an @Any annotated field
